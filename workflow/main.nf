...
process UMI_INJECTOR {
    ...
    script:
    def threads = 1 + (Math.floor((task.cpus - 6).div(2)) * (1 - Math.floor(2.div(1 + 2 ^ (task.cpus - 7)))))
    """
        LC_ALL=C umi-injector.sh --in1=${read_triplets[0]} --umi=${read_triplets[1]}  --in2=${read_triplets[2]} \
