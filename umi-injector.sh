...
arg_invalid() { echo -e "\x01\033[0;31m\x02 $* \x01\033[0m\x02" >&2; exit 2; }  # print complain to stderr and exit with error
arg_required() { if [ -z "$OPTARG" ]; then arg_invalid "No argument was provided for the --$OPT option"; fi; }
arg_requirevalid() { if [ ! -e "$OPTARG" ]; then arg_invalid "Argument to --$OPT option is not a valid file"; fi; }
